    ...
    <div className="fixed inset-0 z-[100] bg-[#050b14] flex flex-col items-center justify-center p-4 text-center overflow-hidden">
      ...
      <div className="animate-scale-in flex flex-col items-center max-w-2xl w-full relative z-10">
        ...
        <div className="relative mb-4 group">
            ...
            <div className="relative bg-white p-1 rounded-full shadow-2xl w-24 h-24 flex items-center justify-center overflow-hidden">
            {!logoError ? (
                <img 
        ...
        {/* Marca LUMINOSA */}
        <h1 className="text-4xl md:text-5xl font-black text-white mb-1 tracking-tight leading-tight drop-shadow-[0_0_15px_rgba(255,255,255,0.5)]">
          SolarTek<span className="text-green-400 drop-shadow-[0_0_15px_rgba(74,222,128,0.8)]">Pro</span>
        </h1>
        <p className="text-xs md:text-sm text-cyan-300 font-bold tracking-[0.3em] uppercase mb-6 drop-shadow-[0_0_5px_rgba(34,211,238,0.8)]">
          Energias Renováveis
        </p>

        {/* Títulos de Boas Vindas */}
        <div className="space-y-1 mb-6">
          <h2 className="text-base md:text-lg font-medium text-gray-400 tracking-wide">
            Bem-vindo ao Moderno
          </h2>
          <h3 className="text-xl md:text-2xl font-bold text-white drop-shadow-[0_0_10px_rgba(255,255,255,0.3)]">
            Sistema de Gestão de Clientes
          </h3>
        </div>

        {/* Missão - Dark Glass */}
        <div className="relative group mb-8 max-w-lg w-full">
          ...
          <div className="relative bg-slate-900/80 backdrop-blur-md px-6 py-4 rounded-xl border border-white/10 shadow-2xl">
            <p className="text-gray-200 text-sm md:text-base font-medium leading-relaxed italic text-shadow-sm">
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      if (currentScrollY > lastScrollY && currentScrollY > 100) {
        setShowFooter(false); // Rolar para baixo -> Esconder
      } else {
        setShowFooter(true); // Rolar para cima -> Mostrar
      }
      ...
      <div className="container mx-auto px-2 py-6 max-w-[95%] pb-20 relative z-10">
        <header className="text-center mb-6">
          <div className="mb-4 flex flex-col items-center justify-center">
            {/* Cabeçalho Glassmorphism Compacto */}
            <div className="flex flex-col md:flex-row items-center gap-4 bg-slate-900/60 backdrop-blur-xl px-6 py-3 rounded-3xl shadow-[0_0_40px_rgba(0,0,0,0.3)] border border-white/5 w-full max-w-3xl mx-auto hover:border-white/10 transition-all duration-500">
              ...
              <div className="relative flex-shrink-0 group cursor-pointer z-50" onClick={handleReload} title="Recarregar Sistema">
                ...
                <div className="relative bg-white p-1 rounded-full w-16 h-16 flex items-center justify-center overflow-hidden border-2 border-slate-900">
                  {!logoError ? (
                    <img 
              ...
              <div className="text-center md:text-left flex-grow">
                <h1 className="text-2xl md:text-4xl font-black text-white tracking-tight leading-none mb-1 drop-shadow-lg">
                ...
                <div className="flex items-center justify-center md:justify-start gap-3 mt-1">
                   ...
                   <p className="text-[10px] md:text-xs font-bold text-cyan-300 tracking-[0.3em] uppercase shadow-black drop-shadow-sm">
                     Energias Renováveis
                   </p>
        ...
        {/* NAV BAR GLASS */}
        <nav className="flex justify-center mb-6 sticky top-4 z-50 no-print">
          <div className="bg-slate-900/80 backdrop-blur-md rounded-2xl shadow-[0_0_20px_rgba(0,0,0,0.4)] p-1.5 flex space-x-1 border border-white/10">
            <button 
              ...
              className={`px-4 py-2 rounded-xl text-sm font-bold transition-all duration-300 flex items-center gap-2 ${view === 'form' && !clienteEditando ? 'bg-blue-600 text-white shadow-[0_0_15px_rgba(37,99,235,0.5)]' : 'text-slate-400 hover:text-white hover:bg-white/5'}`}
            >
              <span>💼</span> Novo Cliente
            </button>
            <button 
              onClick={() => setView('list')}
              className={`px-4 py-2 rounded-xl text-sm font-bold transition-all duration-300 flex items-center gap-2 ${view === 'list' ? 'bg-blue-600 text-white shadow-[0_0_15px_rgba(37,99,235,0.5)]' : 'text-slate-400 hover:text-white hover:bg-white/5'}`}
            >
              <span>📋</span> Lista de Clientes
            </button>
            <button 
              onClick={() => setView('financial')}
              className={`px-4 py-2 rounded-xl text-sm font-bold transition-all duration-300 flex items-center gap-2 ${view === 'financial' ? 'bg-blue-600 text-white shadow-[0_0_15px_rgba(37,99,235,0.5)]' : 'text-slate-400 hover:text-white hover:bg-white/5'}`}
            >
            ...
            <div className="bg-slate-900/50 backdrop-blur-sm rounded-2xl shadow-2xl border border-white/5 overflow-hidden">
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-left">
                  <thead className="bg-black/20 text-slate-300 font-bold border-b border-white/5 text-xs uppercase tracking-wider">
                    <tr>
                  ...
                  <tbody className="divide-y divide-white/5 text-sm">
                    {clientes.length === 0 ? (
                      <tr><td colSpan={7} className="p-10 text-center text-slate-400 italic">Nenhum cliente cadastrado.</td></tr>
                    ) : (
                      clientes.map(cliente => (
                        <tr key={cliente.id} className="hover:bg-white/[0.05] transition-colors group cursor-pointer" onClick={() => setClienteVisualizando(cliente)}>
                          <td className="p-4 font-bold text-white text-base">{cliente.nome}</td>
                          <td className="p-4 text-slate-300 font-mono">{cliente.cpf}</td>
                          ...
                          <td className="p-4 text-right flex justify-end gap-2 opacity-100 md:opacity-80 md:group-hover:opacity-100 transition-opacity" onClick={(e) => e.stopPropagation()}>
                            <button 
                              type="button"
                              onClick={(e) => { e.stopPropagation(); setClienteVisualizando(cliente); }} 
                              className="bg-slate-800 hover:bg-slate-700 text-slate-300 hover:text-white p-2 rounded-lg transition-all border border-white/10" 
